 ...
 * @since 3.3
 */
public class FKM<T extends Clusterable> extends Clusterer<T> {
     ...
     */
    public FKM(final int k, final double fuzziness) throws NumberIsTooSmallException {
     ...
     * @throws NumberIsTooSmallException if {@code fuzziness <= 1.0}
     */
    public FKM(final int k, final double fuzziness,
...
//        final int pointDimension = points.get(0).getPoint().length;
        for (int i = 0; i < k; i++) {
//        	DoublePoint p=new DoublePoint(points.get(i).getPoint());
            clusters.add(new CentroidCluster<T>(points.get(i)));
        }
    ...
    private void updateMembershipMatrix() {
        for (int i = 0; i < points.size(); i++) {
            ...
            double[] val=point.getPoint();
 /*           
            System.out.println("第"+i+"实例");
			for(int u=0; u<val.length;u++){
				System.out.print(val[u]+", ");
			}
			System.out.println();
*/			
            ...
            for (int j = 0; j < clusters.size(); j++) {
                double sum = 0.0;
  /*              
                double[] cval=clusters.get(j).getCenter().getPoint();
                System.out.println("第"+j+"聚类中心");
				for(int u=0; u<cval.length;u++){
					System.out.print(cval[u]+", ");
				}
				System.out.println();
*/				
    ...
    private void initializeMembershipMatrix() {
        System.out.println("init membership matrix");
        for (int i = 0; i < points.size(); i++) {
            for (int j = 0; j < k; j++) {
                membershipMatrix[i][j] =0.01d + random.nextDouble();
            }
            membershipMatrix[i] = MathArrays.normalizeArray(membershipMatrix[i], 1.0);
        /*    
            for(int m=0; m<membershipMatrix[i].length; m++){
            	System.out.print(membershipMatrix[i][m]+" ");
            }
            System.out.println();
          */   
        }
...
//		random.setSeed(10);
		...
		Instances instances=ld.loadData("/home/ucas/software/aluminium-electrolysis/CSV日报/CSV一厂房日报/101-FKM-normalize.arff");
//		Instances instances=ld.loadData("/home/ucas/software/aluminium-electrolysis/CSV日报/CSV一厂房日报/101-FKM.arff");
